...
#            self.data=pd.concat([data1,self.data],ignore_index=True)
    ...
    def get_zh_data_from_east(self,num,code,freq,adjustflag):
        ...
        url='http://push2his.eastmoney.com/api/qt/stock/kline/get?fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&beg={}&end=20500101&ut=fa5fd1943c7b386f172d6893dbfba10b&rtntype=6&secid={}.{}&klt={}&fqt={}&cb=jsonp1666753798836'.format(self.start_date,num,code,freq,adjustflag)
    ...
    def get_us_data_from_east(self,code,freq,adjustflag):
        day=1080
        url='http://45.push2his.eastmoney.com/api/qt/stock/kline/get?cb=jQuery3510640595823038469_1667035995811&secid={}&ut=fa5fd1943c7b386f172d6893dbfba10b&fields1=f1%2Cf2%2Cf3%2Cf4%2Cf5%2Cf6&fields2=f51%2Cf52%2Cf53%2Cf54%2Cf55%2Cf56%2Cf57%2Cf58%2Cf59%2Cf60%2Cf61&klt={}&fqt={}&end=20500101&lmt={}&_=1667035995818'.format(code,freq,adjustflag,day)
        data=requests.get(url,headers=self.headers).content.decode()
        json_data=re.findall('{"rc":.*]}}',data)[0]
        json_data=json.loads(json_data)
        data_list=json_data['data']['klines']
...
#    def get_hk_data_from_akshare(self,code,freq,adjustflag):
#        self.start_date=(datetime.date.today()+datetime.timedelta(days=-365)).strftime("%Y%m%d")
#        if freq=='weekly':
#            self.start_date=(datetime.date.today()+datetime.timedelta(days=-1260)).strftime("%Y%m%d")
#        elif freq=='monthly':
#            self.start_date=(datetime.date.today()+datetime.timedelta(days=-5400)).strftime("%Y%m%d")
#        self.data = ak.stock_hk_hist(symbol=code,
#                                    start_date=self.start_date,
#                                    end_date=self.end_date,
#                                    period=freq,
#                                    adjust=adjustflag)
...
#    def get_us_data_from_akshare(self,code,freq,adjustflag):
#        self.start_date=(datetime.date.today()+datetime.timedelta(days=-365)).strftime("%Y%m%d")
#        if freq=='weekly':
#            self.start_date=(datetime.date.today()+datetime.timedelta(days=-1260)).strftime("%Y%m%d")
#        elif freq=='monthly':
#            self.start_date=(datetime.date.today()+datetime.timedelta(days=-5400)).strftime("%Y%m%d")
#        self.data=ak.stock_us_hist(symbol=code,
#                                    start_date=self.start_date,
#                                    end_date=self.end_date,
#                                    period=freq,
#                                    adjust=adjustflag)
...
#        self.deal_with_data(self.data)
    ...
    def get_other_index(self,code,freq):
        days=1365
        url='http://push2his.eastmoney.com/api/qt/stock/kline/get?secid={}&fields1=f1%2Cf2%2Cf3%2Cf4%2Cf5&fields2=f51%2Cf52%2Cf53%2Cf54%2Cf55%2Cf56%2Cf57%2Cf58%2Cf59%2Cf60%2Cf61&lmt={}&klt={}&fqt=1&end=30000101&ut=4f1862fc3b5e77c150a2b985b12db0fd&cb=cb_1665195942532_53072773&cb_1665195942532_53072773=cb_1665195942532_53072773'.format(code,days,freq)
        df=requests.get(url,headers=self.headers).content.decode()
        json_data=re.findall('({"rc":.*]}})',df)[0]
        json_data=json.loads(json_data)['data']['klines']
