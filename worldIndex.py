...
class WorldIndex():
    ...
    def getAllIndex(self):
        ...
        data=DataFrame(requests.get(url,headers=self.headers).json()['data']['diff'])
    ...
    def get_futures_data(self):
        ...
        df=pd.DataFrame(requests.get(url,headers=self.headers).json()['list'])
        df=df[['name','p','zdf']]
...
#        df.index = pd.RangeIndex(start=0, stop=len(df), step=1)
    ...
    def get_time_share_tick(self,num,code):
        ...
        df =requests.get(url,headers=self.headers).json()['data']
...
#        return (preClose,data_list)
    ...
    def get_time_share_chart_data5(self,num,code):
        ...
        json_data=requests.get(url5,headers=self.headers).json()['data']
        preClose=json_data['preClose']
        data_list=json_data['trends']
        return (preClose,data_list)
